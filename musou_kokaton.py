...
class Bomb(pg.sprite.Sprite):
    ...
    def __init__(self, emy: "Enemy", bird: Bird):
        ...
        self.speed = 6
        self.state = "active"  # bomb.stateをactiveに設定
...
class NeoBeam:
    ...
    def __init__(self, bird: Bird, num: int):
        self.bird = bird
        self.num = 100 // (num - 1) # 個数に応じた数値を設定する

    def gen_beams(self):
        b_lst = [] # ビームの角度が入るからリストを生成
        for i in range(-50, 51, self.num):
            b_lst.append(i)
        return [Beam(self.bird, i) for i in b_lst] # 角度ごとにBeamインスタンスを作りリストで返す。
...
class EMP:
    ...
    def __init__(self, emys: Enemy, bombs: Bomb, screen:pg.Surface):
        """
        敵機と爆弾を無効化設定し、画面全体を黄色く覆う矩形の生成
        引数1 emys:Enemyインスタンスのグループ
        引数2 bombs: Bombインスタンスのグループ
        引数3 screen: 画面surface
        """
        self.life = 20  # 黄色い矩形の描画時間
        self.screen = screen
        for emy in emys:
            emy.interval = float("inf")  # 敵機の爆弾発射間隔を無限に設定
    ...
    def update(self):
        ...
        if self.life > 0:
            self.life -= 1
            self.screen.blit(self.yellow, [0,0])  # self.lifeが正の間のみ黄色い矩形を描画
...
class Shield(pg.sprite.Sprite):
    ...
    def __init__(self, bird: Bird, life: int):
        ...
        防御壁に関するクラス
        引数1 bird：こうかとん(bird)
        引数2 life：防御壁が表示されるフレーム数
        """
        super().__init__()
        self.life = life
        self.image = pg.Surface((20, bird.rect.height * 2))  # Surfaceを生成
        pg.draw.rect(self.image, (0, 0, 255), (0, 0, 20, bird.rect.height * 2))  # Surfaceにrectをdraw
        vx, vy = bird.dire  # こうかとんの向きを取得
        angle = math.degrees(math.atan2(-vy, vx))  # 角度を求める
        self.image = pg.transform.rotozoom(self.image, angle, 1.0)  # 求めた角度から長方形を回転
        self.image.set_colorkey((0, 0, 0))  # 背景色を透明に
        self.rect = self.image.get_rect()
        self.rect.centerx = bird.rect.centerx + bird.rect.width * vx  # rect中心座標xを設定
        self.rect.centery = bird.rect.centery + bird.rect.height * vy  # rect中心座標yを設定
...
def main():
    ...
    neobeam = NeoBeam(bird, 5) # 弾幕のインスタンスを生成、右側が弾幕の個数
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_e and score.value > 20:
                ...
                score.value -= 20
            if event.type == pg.KEYDOWN and event.key == pg.K_s and score.value >= 50 and shield_tmr == 0:
                shields.add(Shield(bird, 400))  # 防御壁生成
